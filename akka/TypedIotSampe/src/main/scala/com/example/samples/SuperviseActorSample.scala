...
object MainSupervise {
  def apply(): Behavior[String] =
    Behaviors.setup(context => new MainSupervise(context))
}

class MainSupervise(context: ActorContext[String]) extends AbstractBehavior[String](context) {
  override def onMessage(msg: String): Behavior[String] =
    msg match {
      case "start" =>
...
object SuperviseActorSample extends App {
  val testSystem = ActorSystem(MainSupervise(), "testSystem")
  testSystem ! "start"
}
